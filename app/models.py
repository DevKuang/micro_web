...
class PostClassification(db.Model):
    __tablename__='classifications'
    id=db.Column(db.Integer,primary_key=True)
    name=db.Column(db.String(64),unique=True)
    posts=db.relationship('Post',backref='classification',lazy='dynamic')

    @staticmethod
    def insert_classifications():
        classifications=[u'Flask',u'Python',u'阅读',u'随笔',u'问与答',u'其他',u'算法']
        for c in classifications:
            classification=PostClassification.query.filter_by(name=c).first()
            if classification is None:
                classification=PostClassification(name=c)
...
class User(UserMixin,db.Model):
    ...
    def is_following_by(self,user):
        return self.followers.filter_by(
            followers_id=user.id).first() is not None
    @property
